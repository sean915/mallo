...
`      <h2>어떤 업무를 도구로 바꿀까요?</h2>
      <p>템플릿을 고르거나 엑셀·PDF·사진을 붙이고 필요한 흐름을 적어 주세요. 말로가 입력, 저장, 검색까지 갖춘 업무 도구로 정리합니다.</p>`
);
...
`      <div class=${q}empty${q} id=${q}empty${q}>
        ...
        <h3>업무를 설명하면 바로 확인할 수 있어요</h3>
        <p>왼쪽에서 템플릿을 고르거나 아래 입력창에 필요한 일을 적어 주세요.<br>완성된 도구가 이곳에 바로 나타납니다.</p>
      </div>`
);
...
`      <div class=${q}attach-list hidden${q} id=${q}attachList${q}></div>
      <div class=${q}hint${q}>한국어로 편하게 적으세요 · 엑셀/PDF/사진은 참고자료로 첨부할 수 있어요</div>`
);
