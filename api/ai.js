...
// 호출 성공 시 말로 잔액 990원을 차감하고, 업스트림 실패/빈 응답은 자동 복구한다.
import { json, env, getUser, sb, AI_FEATURE_PRICE } from './_lib.js';
...
function readAiConfig() {
  ...
  if (!key) {
    ...
    modelFallback = env('LLM_FALLBACK_MODEL', '');
  }
...
async function restoreAiCredit(uid, quota) {
  ...
  try {
    const res = await sb('rpc/restore_generation', {
      method: 'POST',
      body: JSON.stringify({ uid, p_source: source, p_cost: Number(quota?.cost || AI_FEATURE_PRICE) }),
    });
    if (!res.ok) {
      const detail = await res.text().catch(() => '');
  ...
  if (!quota.allowed) {
    ...
    const message = quota.reason === 'busy'
      ...
      : quota.reason === 'server_error'
        ? '잔액 확인 중 오류가 났어요. 잠시 후 다시 시도해 주세요.'
        : `AI 기능은 1회 ${AI_FEATURE_PRICE.toLocaleString('ko-KR')}원이에요. 잔액을 충전하면 계속 사용할 수 있습니다.`;
  ...
  try {
    ...
    if (!res.ok) {
      ...
      const friendly = res.status === 429
        ? 'AI 사용량이 많아요. 잔액은 차감되지 않았어요. 잠시 후 다시 시도해 주세요.'
        : 'AI 처리 중 문제가 발생했어요. 잔액은 차감되지 않았어요. 잠시 후 다시 시도해 주세요.';
    ...
    if (!text) {
      await restoreAiCredit(user.id, quota);
      return json({ error: 'AI가 답을 만들지 못했어요. 잔액은 차감되지 않았어요. 다시 시도해 주세요.' }, 503);
    }

    return json({ text, remaining: quota.remaining });
  } catch (e) {
    await restoreAiCredit(user.id, quota);
    return json({ error: 'AI 연결에 실패했어요. 잔액은 차감되지 않았어요. 잠시 후 다시 시도해 주세요.' }, 503);
  }
}
