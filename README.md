...
사용자 브라우저 (index.html)
  ...
  └─ POST /api/generate (Vercel Edge Function)
       ├─ 로그인 검증 (Supabase)
       ├─ 무료 체험/AI 크레딧 잔액 검사·차감 (Postgres 함수, 원자적)
       ├─ 사용자 요청을 내부 정제 프롬프트로 변환
       └─ LLM 호출 (서버 환경변수의 키) → SSE 스트림으로 실시간 전달
```

컨셉: **한 번 호출하면 원큐로 프로그램(SW) 하나를 만든다.** 사용자는 만료 없는 AI 크레딧을 단건 결제로 충전하고, 프로그램 생성/수정 또는 생성된 도구 안의 AI 기능 사용 1회마다 크레딧 1건이 차감됩니다.

## 가격 정책

| AI 크레딧 | 가격 | 할인률 | 표시 |
|---|---:|---:|---|
| 1건 | 990원 | - | 도구 생성/수정 또는 AI 기능 1회 |
| 5건 | 4,700원 | 5% | 5% 할인 |
| 10건 | 8,900원 | 10% | 10% 할인 |
| 30건 | 25,200원 | 15% | 15% 할인 |

- 무료 체험: 비로그인 기기당 1건 + 로그인(가입) 시 2건
- AI 크레딧 만료 없음
- 결제 후 7일 이내 미사용분 환불
- 생성 실패, AI 응답 실패, 중복결제 등 시스템 오류는 크레딧 자동 복구 또는 환불
...
| `api/generate.js` | 프롬프트 정제 + LLM 프록시 + 크레딧 차감/실패 복구 + SSE 스트리밍 |
| `api/ai.js` | 생성된 도구의 AI 기능 중계 + 크레딧 차감/실패 복구 |
| `api/me.js` | 내 무료 체험/AI 크레딧 잔액 조회 |
| `api/purchase.js` | 포트원 단건결제 검증 + AI 크레딧 충전 |
| `api/config.js` | 프론트에 Supabase 공개 설정·크레딧 가격·결제 준비 상태 전달 |
| `api/_lib.js` | 공용 유틸 + 크레딧 가격 단일 출처 |
| `supabase.sql` | DB 스키마 + 크레딧 충전/차감/복구 함수 |
...
5. Vercel 환경변수에 아래 3개를 추가
   ...
   - `PORTONE_CHANNEL_KEY`
   - `PORTONE_API_SECRET`
6. 테스트 모드에서 1건/5건 결제 → `/api/purchase`가 PortOne 결제 조회 후 `add_credits`로 크레딧을 충전하는지 확인
7. 실연동 전환 후 실제 990원 결제/취소/부분 환불 운영 플로우 확인
...
- [ ] 생성 실패 또는 AI 응답 실패 시 크레딧 복구 테스트
